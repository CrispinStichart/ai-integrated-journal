...
| API boundary | `finalizeWebmBytes` owns the platform-neutral repair and `finalizeWebmBlob` is the only browser adapter. Callers no longer inject or import an implementation. Parsing, timeline/metadata calculation, and encoding use separate immutable interfaces. |
| Source modernization | The initially imported snapshot under `src/upstream` was retained through characterization, then replaced by strict TypeScript modules under `src/`. The build is ESM/NodeNext, all relative imports include `.js`, and no source is excluded from type checking, linting, or formatting. |
| Byte representation | The fork uses `Uint8Array`, `DataView`, `TextEncoder`/`TextDecoder`, and `Blob`. Parsed unknown elements and every Cluster byte range are retained without interpretation and copied byte-for-byte. Mutable reader state and event-emitter APIs were removed. |
| Runtime dependencies | The upstream `buffer`, `ebml-block`, `events`, and `int64-buffer` dependencies were all removed. Their small required operations are expressed with standard byte/Web APIs, avoiding browser polyfills and CommonJS interop without expanding the package's media scope. |
| Failure contract | Unsupported input, invalid EBML, numeric overflow, truncated data, and resource-policy violations have stable typed error classes and discriminating codes. Validation completes before a successful result is exposed and never mutates caller-owned input. |
| Characterization | Synthetic fixture generators, structural assertions, a sanitized regression analogue, semantic baselines, and one golden SHA-256 are recorded in the package tests and `CHARACTERIZATION.md`. The modern implementation retains the golden output and adds byte-preservation coverage for opaque elements. |
| Resource boundaries | Whole-file repair is capped at 128 MiB input and 8 MiB retained/generated metadata, parser recursion at 32 levels, and parsed elements at 500,000. These are operation limits, not recording policy. Invalid offsets/sizes and truncation fail deterministically; VINT and malformed-tree property tests exercise the boundaries. See `RESOURCE-LIMITS.md`. |
