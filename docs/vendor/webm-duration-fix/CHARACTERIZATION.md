...
| Malformed/truncated | Invalid zero element ID, and a valid stream truncated within its final SimpleBlock | Rejects the malformed ID with `Unrepresentable length`. The legacy implementation does **not** reject the truncated case: it rewrites complete metadata and carries the incomplete tail through. This unsafe behavior is recorded, not endorsed; Task 5 is responsible for deterministic truncated-input rejection. |
