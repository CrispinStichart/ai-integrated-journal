...
- Status: Acceptance and product edge-case audit completed through Phase 8 Task 51
- Date: 2026-08-30
...
| RET-006–007 | Domain/contracts, real-PostgreSQL retention integration, worker retry/idempotency, IndexedDB purge, browser API tests, export invalidation, and backup tooling cover grace, tombstone-first bounded deletion across rows/revisions/blobs/staging/text/vectors/caches/outboxes/raw responses, anti-resurrection, configured checkpoint gating, and newest-ledger restore replay. Task 52 adds destructive recovery fault drills. | 47–49, 52 | Complete local release | Partial |
...
| EC-17 | Processor/API outage | `apps/web/test/journal-day-view.test.ts`, `[CAP-001][STATE-002][STATE-003][AC-001][AC-003]`, keeps failed-stage audio playable and typed text editable with targeted retry; `apps/api/test/text-journal-milestone.integration.ts` repeats source use with provider unavailable and worker stopped. | Verified |
