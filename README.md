...
OpenAI is registered in the API and worker for structured generation and speech-to-text. To use it, configure `AI_CREDENTIAL_ENCRYPTION_KEY` for both processes, then open **Settings**, enter an OpenAI API key and a model ID for each capability, accept the disclosure, and enable OpenAI. Text models must support the Responses API with JSON-schema output and the requested generation parameters (the built-in text jobs request `temperature: 0`). No model is selected automatically.

Audio supports `gpt-transcribe`, `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`, and `whisper-1`. GPT transcription returns text with unavailable timing metadata represented as `unknown`; Whisper requests segment and word timestamps. Uploads are limited to 25,000,000 audio bytes in MP3, MP4/M4A, MPEG/MPGA, WAV, or WebM format. Larger or unsupported recordings fail without automatic splitting or transcoding. Embeddings, speech synthesis, and realtime audio are not implemented.

The adapter uses the [Responses API](https://developers.openai.com/api/reference/typescript/resources/responses/methods/create) with `store: false` and non-strict JSON-schema output to retain optional fields in existing processor schemas. Every result is still validated locally. Speech uses the [file transcription API](https://developers.openai.com/api/docs/guides/speech-to-text). Requests time out after two minutes and job retries remain owned by the queue. See [OpenAI data controls](https://developers.openai.com/api/docs/guides/your-data) for provider retention and training policies; local raw-response retention remains separate.

When adding another adapter, register it in both composition roots and extend the owner-scoped resolver as needed. Adding an adapter alone does not wire job execution.
...
The worker's server-only `OwnerProviderResolver` reads owner configuration at each operation, verifies current disclosure acceptance, and decrypts only the selected provider's credential. It derives owners from canonical recording or processor targets, or the grounded-answer owner. Absent an explicit provider selection, it selects OpenAI; unknown explicit providers fail without fallback. The configured capability model is authoritative. Missing models or credentials fail before an external request. Keep this boundary when adding providers, and never place credentials in settings responses or provenance. Embedding execution remains unconfigured.
...
Credentials are write-only through the API/UI and are excluded from responses, logs, audits, exports, and backups. The deployment key must be stored separately from journal and backup data. Losing or changing it makes stored credentials unusable, so they must be entered again. The API writes encrypted credentials and the worker uses a narrow owner/provider-scoped read and authenticated decrypt path for each operation. Both processes need the same deployment key. See [`settings-service.ts`](apps/api/src/settings-service.ts) and the [configuration reference](docs/configuration.md).
